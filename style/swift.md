# iOS Style Guide (Swift)

## Introduction

This style guide is based on the [official raywenderlich.com Swift style guide](https://github.com/raywenderlich/swift-style-guide) but has some changes to better accommodate the practices of our team. We would like to thank [Ray and his team](https://www.raywenderlich.com), and all of those who contributed to their style guide, for creating it in the first place.

Another source of style guidelines is the [API Design Guidelines](https://swift.org/documentation/api-design-guidelines/#include-words-to-avoid-ambiguity). These guidelines are also reflected in this document.
...
For some non-additive changes such as misspellings, grammar or compiler errors, changes to this guide can be performed by simply issuing a pull request. If case the changes are deemed more "controversial", open an issue to clarify the change and to allow discussion within the team so that other members of our team can participate in the discussion. We thank any and all such contributions in advance!
...
- [using `UpperCamelCase` for names of types and protocols. Everything else is `lowerCamelCase`.](https://swift.org/documentation/api-design-guidelines/#follow-case-conventions)
...
- casing acronyms and initialisms uniformly up or down. Use `ID` when part of a type but `Id` when part of an identifier's name
...
* Use Xcode's defaults values. The default settings are to indent using 4 spaces rather than tabs to conserve space and help prevent line wrapping. Be sure to set this preference in Xcode and in the Project settings as shown below:
...
* Long lines should be wrapped at around 170 characters. This is just a reference. A hard limit is intentionally not specified. 
**Note:** Current limit defined in swiftlint is bigger than 170.
...
Use `@available(*, unavailable)` when using unimplemented init-with-coder initialisers.
...
For functions with long signatures, more than the recommended length, put each parameter on a new line and add an extra indent on subsequent lines.
...
let hypotenuse = side * sqrt2 // what is root2?
```
...
A common swiftlint file should be used on all codebases in order to achieve consistency. Due to some legacy code, that is still a **work in progress**.

Please, check each project's swiftlint file to confirm the rules in place.